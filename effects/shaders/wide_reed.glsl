...
void main() {
    ...
    uv.x += sin(uv.y * 4.6 + uTime * 0.8) * 0.018 * uIntensity;
    ...
    float refractX = (normal * abs(normal) * 0.068 + sin(uv.y * 12.0 + uTime * 1.5) * 0.010) * uIntensity;
    float refractY = (sin(floor(uv.x * count) * 0.7 + uTime * 0.7) * 0.0070) * uIntensity;
    ...
    float seam = 1.0 - smoothstep(0.36, 0.50, abs(normal));
    float edge = smoothstep(0.26, 0.0, abs(normal));
    vec3 color = mix(base, glass, clamp((0.34 + 0.30 * mask) * uIntensity, 0.0, 0.64));
    color += mask * 0.080 * uIntensity + edge * 0.05 * uIntensity;
